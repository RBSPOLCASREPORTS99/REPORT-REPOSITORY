...
export default function SalesTable({
  ...
  if (rows.length === 0) return <p className="text-slate-400">No sales detail for this comparison.</p>;
          ...
          {sorted.map((r, i) => {
            ...
            return (
              <tr key={r.item + i} className="border-b border-slate-50">
                ...
                <td className="px-2 py-2.5 text-slate-400">{r.uom}</td>
                <td className="px-3 py-2.5 text-right tabular-nums text-slate-500">{qty(r.prior)}</td>
                <td className="px-3 py-2.5 text-right tabular-nums font-medium text-slate-900">{qty(r.current)}</td>
