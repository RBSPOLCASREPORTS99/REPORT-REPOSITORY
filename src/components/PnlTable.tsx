      ...
      <table className="min-w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-700 text-xs font-medium text-slate-400 dark:text-slate-500">
            <th className="sticky left-0 bg-white dark:bg-slate-800 px-4 py-2 text-left">Line item</th>
          ...
          {lines.map((line) => {
            ...
            return (
              <tr key={line.key} className={`border-b border-slate-50 dark:border-slate-700/60 ${rowCls}`}>
                <td className={`sticky left-0 px-4 py-2.5 text-left ${bold ? 'bg-slate-50 dark:bg-slate-700 font-semibold text-slate-900 dark:text-slate-100' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>
                  {line.label}
                </td>
                {line.isPct ? (
                  <>
