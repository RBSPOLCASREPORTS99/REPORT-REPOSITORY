...
// Gross Sales, Gross Income, Total Expense, and Net Income from Operations.
export async function fetchTrend(buCode: string, limit = 12): Promise<TrendPoint[]> {
  const { data, error } = await supabase
    ...
    .eq('bu_code', buCode)
    .in('line_item', ['gross_sales', 'gross_income', 'total_expense', 'net_income_ops']);
  if (error) throw error;
  ...
  for (const row of (data ?? []) as unknown as { amount: number; line_item: string; report_ranges: { label: string; kind: string; period_end: string } }[]) {
    ...
    if (!byRange.has(k)) {
      byRange.set(k, { label: row.report_ranges.label, periodEnd: k, grossSales: 0, grossIncome: 0, totalExpense: 0, netIncomeOps: 0 });
    }
